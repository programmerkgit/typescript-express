...
//export class ModelApiFactory<T extends Model<T>>

//(model: (new () => T) & typeof Model): ModelApi
// {

export class ModelApiFactory<T extends Model<T>> {
    /**/
    private readonly model: (new () => T) & typeof Model;
    ...
    create: RequestHandler = ((req, res, next) => {
        ...
        const values = req.body;
        this.model.create(values, options).then((instance: any) => {
            res.status(201);
    ...
    delete: RequestHandler = (req, res, next) => {
        const id = req.params.id;
        this.model.findByPk(id).then((instance: any) => {
            if (instance) {
